...
function printInstrucciones(name) {
    console.log(name + ', éstas son las instrucciones del juego:');
    console.log('Contesta con una palabra a la definición dada.')
    console.log('Si no sabes la respuesta, escribe: PASAPALABRA.')
    console.log('Si quieres salir del juego, escribe: END.')
    console.log('BUENA SUERTE!!')
    console.log('-----------------------------------------')
}

//seleccionamos una pregunta por letra
function createPlayerQuestions(questions) {
    let playerQuestions = []
    for (i= 0; i < questions.length ; i++) {
        var randomNum = Math.floor((Math.random() * 4));
        playerQuestions.push(questions[i][randomNum])
    }
...
function AskQuestion (question) {
    var respuesta = prompt(question);
    return respuesta.toLowerCase();
}

// juego y modificamos status dependiendo de la respuesta
function game(player) {
    
    while ((isFinished(player.questions) == false)) {
        for (let i = 0; i < player.questions.length; i++) {
            if (player.questions[i].status === 0) {
                respuesta = AskQuestion (player.questions[i].question)
                switch (respuesta) {
                    case 'end':
                        player.rank = false;
                        return player;
                    case 'pasapalabra': 
                        ...
                        break;
                    case player.questions[i].answer:
                        player.questions[i].status = 1;
                        alert('CORRECTO!');
                        break;
                    default:
                        player.questions[i].status = 2;
...
// comprobar si todas las respuestas se han respondido
function isFinished(playerQuestions) {
    for (let i= 0; i < playerQuestions.length; i++) {
        if (playerQuestions[i].status === 0) {
        return false} 
    }
    return true
}
...
// función inicial para construir usuario 
function inicio(questions){
    console.log('-----------------------------------------')
    var player = {
        name: hola(),
        questions: createPlayerQuestions(questions),
        rank: -1,
        rightAnswers: 0,
        wrongAnswers: 0
    }
...
// contamos respuestas correctas
function countRightPoints(questions) {
let counterRight = 0;
...
//contamos respuestas incorrectas
function countWrongPoints(questions) {
    let counterWrong = 0;
    for (let i = 0; i < questions.length ; i++) {
        if (questions[i].status == 2) {counterWrong = counterWrong + 1}
        }
    return counterWrong;
}
 
//mostramos número de respuestas correctas e incorrectas
function printResults(name, rightAnswers, wrongAnswers){
console.log(`${name}, you finished the game!`)
console.log(`You have ${rightAnswers} correct answers.`)
console.log(`You have ${wrongAnswers} wrong answers.`)
console.log('-----------------------------------------')
}

//construimos el orden del ranking
function doRanking (players, leaderBoard) {
   
   if (leaderBoard.length == 0) {leaderBoard.push(0)}
    else {
        newPlayer = players[players.length - 1];
        for (let i = 0; i < leaderBoard.length; i++) {
            if (newPlayer.rightAnswers > players[leaderBoard[i]].rightAnswers) {
                leaderBoard.splice(i,0,players.length -1)
            return leaderBoard;}   
        } leaderBoard.push(players.length -1)
    }
return leaderBoard;
}

//imprimimos ranking
function printRanking(leaderBoard, players) {
    console.log('Éste es el ranking actual:')
    for (let i = 0; i < leaderBoard.length; i++) {
        console.log('Nombre: '  + players[leaderBoard[i]].name + '. Puntos: ' + players[leaderBoard[i]].rightAnswers)
    }
    console.log('-----------------------------------------')
}
...
function pasapalabra() {
    ...
    var questions = [
        [
            ...
            { letter: "a", answer: "anglosajón", status: 0, question: "CON LA A. Dicho de una persona: De procedencia y lengua inglesas" },
        ...
        [
            ...
            { letter: "b", answer: "beata", status: 0, question: "CON LA B. Dicho de una persona: Beatificada por el papa" },
        ...
        [
            ...
            { letter: "c", answer: "cartabón", status: 0, question: "CON LA C. Plantilla de madera, plástico u otro material, en forma de triángulo rectángulo escaleno, que se utiliza en dibujo" },
        ...
        [
            ...
            { letter: "d", answer: "dragón", status: 0, question: "CON LA D. Criatura fantástica sobre la que volaba Daenerys Targaryen" },
        ...
        [
            ...
            { letter: "e", answer: "emanación", status: 0, question: "CON LA E. Acción y efecto de emanar" },
        ...
        [
            ...
            { letter: "f", answer: "fósil", status: 0, question: "CON LA F. Dicho de una sustancia de origen orgánico o de un resto de organismo: Que está más o menos petrificado" },
        ...
        [
            ...
            { letter: "h", answer: "habitación", status: 0, question: "CON LA H. En una vivienda, cada uno de los espacios entre tabiques destinados a dormir, comer, etc" },
        ],
        [
            ...
            { letter: "i", answer: "intención", status: 0, question: "CON LA I. Determinación de la voluntad en orden a un fin" },
        ...
        [
            ...
            { letter: "j", answer: "jauría", status: 0, question: "CON LA J. Conjunto de perros mandados por el mismo perrero que levantan la caza en una montería" },
            { letter: "j", answer: "jamón", status: 0, question: "CON LA J. Pierna trasera del cerdo, curada o cocida entera" },
        ...
        [
            ...
            { letter: "m", answer: "músculo", status: 0, question: "CON LA M. Órgano compuesto principalmente de fibras contráctiles" },
        ...
        [
            ...
            { letter: "p", answer: "políglota", status: 0, question: "CON LA P. Dicho de una persona: Que habla varias lenguas" },
        ],
        [
            ...
            { letter: "q", answer: "empaquetar", status: 0, question: "CONTIENE LA Q. Hacer paquetes" },
            { letter: "q", answer: "adquirir", status: 0, question: "CONTIENE LA Q. Ganar, conseguir con el propio trabajo o industria" },
        ],
        [
        ...
        [
            ...
            { letter: "t", answer: "tensión", status: 0, question: "CON LA T. Estado de un cuerpo sometido a la acción de fuerzas opuestas que lo atraen" },
        ...
        [
            ...
            { letter: "w", answer: "walkman", status: 0, question: "CON LA W. Reproductor portátil de casetes provisto de auriculares" },
            { letter: "w", answer: "wolfgang", status: 0, question: "CON LA W. Nombre de pila del famoso compositor con apellildo Mozart" },
            { letter: "w", answer: "wasabi", status: 0, question: "CON LA W.  Pasta de color verde y picante que se sirve para acompañar delicias como el sushi o el sashimi" },
            { letter: "w", answer: "weber", status: 0, question: "CON LA W. Unidad de flujo magnético del sistema internacional, equivalente al flujo magnético que, al atravesar un circuito de una sola espira, símboo wb" },
        ],
        [
            { letter: "x", answer: "botox", status: 0, question: "CONTIENE LA X. Toxina bacteriana utilizada en cirujía estética" },
            { letter: "x", answer: "xenófoba", status: 0, question: "CON LA X. Dicho de una persona: Que siente o manifiesta xenofobia" },
            { letter: "x", answer: "méxico", status: 0, question: "CONTIENE LA X. País de América ubicado en la parte meridional de América del Norte" },
            { letter: "x", answer: "xenóin", status: 0, question: "CON LA X. Elemento químico o cuerpo simple, gaseoso, incoloro e inodoro, encontrado en el aire" },
            { letter: "x", answer: "xilófono", status: 0, question: "CON LA X. Instrumento músico compuesto de láminas de madera, sostenidas por hilos de seda o cuerda de tripa, de espesor y longitud tales que, golpeados con un martillo dan notas diferentes" },
        ],
        [
            ...
            { letter: "y", answer: "yacare", status: 0, question: "CON LA Y. Caimán"},
        ],
        [
    ...
    let player = inicio(questions);
    printInstrucciones(player.name);
    player = game(player);
    player.rightAnswers = countRightPoints(player.questions);
    player.wrongAnswers = countWrongPoints(player.questions);
    printResults(player.name, player.rightAnswers, player.wrongAnswers);
    return player
}
...
let leaderBoard = [];
    ...
    do {
        newPlayer = pasapalabra()
        if (newPlayer.rank == true) {
            players.push(newPlayer);
            leaderBoard = doRanking(players,leaderBoard);
            printRanking(leaderBoard, players)
        }
    } while (playAgain())
}
