...
function muestraInfoJuego() {
    alert('¡Bienvenid@ a Pasapalabra!' +
        '\nEl juego consiste en responder correctamente a todas las preguntas con una palabra que, empiece o contenga, ' +
        'la letra que se le indique en cada pregunta. Si responde correctamente, el juego continuará a la siguiente pregunta. ' +
        'Si la respuesta es incorrecta, el juego le indicará el error y le dirá la respuesta correcta. Si responde pasapalabra, ' +
        'el juego continuará y guardará la pregunta para la siguiente ronda.' + '\nEl juego finalizará cuando responda todas las preguntas.');
}
...
function preguntasRandom() {
    questions.forEach(function (element) {
        let numero = Math.floor((Math.random() * (4 - 0 + 1)));
        preguntas.push(element[numero]);
    });
}

function juego() {

    while (preguntas.some(function (element) {
        return element.status === 0
    })) {
        for (let i = 0; i < preguntas.length; i++) {
            if (preguntas[i].status === 0) {
                let respuesta = (prompt(preguntas[i].question))
                switch (respuesta.toLowerCase()) {
                    case preguntas[i].answer:
                        alert('¡¡RESPUESTA CORRECTA!!!')
                        preguntas[i].status = '1';
                        aciertos++
                        break;
                    case 'pasapalabra':
                        alert('Vamos a la siguiente pregunta. En la próxima ronda retomaremos esta pregunta.');
                        break;
                    case 'end':
                        alert('Ha decidido finalizar la partida.')
                        alert(usuario + ' , ha finalizado el juego con ' + aciertos + ' aciertos y ' + fallos + ' fallos.');
                        nuevoRosco();
                        return;
                        break;
                    default:
                        alert('¡NO! RESPUESTA INCORRECTA!')
                        alert('La respuesta correcta es: ' + preguntas[i].answer);
                        preguntas[i].status = '2';
                        fallos++;
                        break;
                }
            }
...
function ranking() {
    console.log(usuario + '. Has completado el rosco de Pasapalabra, con ' + aciertos + ' aciertos ' + fallos + ' fallos.');
    rankingJugadores.push({ usuario: usuario, aciertos: aciertos, fallos: fallos });
    rankingJugadores.sort((a, b) => b.aciertos - a.aciertos);
    console.log('Ranking:');
    for (let i = 0; i < rankingJugadores.length; i++) {
        console.log('Jugador: ' + rankingJugadores[i].usuario + ' aciertos: ' + rankingJugadores[i].aciertos)
    }
...
function nuevoRosco() {
    let opcion = confirm('¿Desea volver a jugar?');
    if (opcion === true) {
        usuario = '';
        aciertos = 0;
        fallos = 0;
