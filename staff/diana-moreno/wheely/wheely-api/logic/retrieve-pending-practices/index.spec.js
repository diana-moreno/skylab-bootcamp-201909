...
const retrievePendingPractices = require('.')
...
describe('logic - retrieve pending practices', () => {
  ...
  it('should succeed on retrieve the student practices', async () => {
    practices = await retrievePendingPractices(studentId)

    expect(practices).to.exist
    expect(practices[0].status).to.equal('pending')
    expect(practices[1].status).to.equal('pending')
  })

  it('should succeed on retrieve the instructor practices', async () => {
    practices = await retrievePendingPractices(instructorId)
  ...
  it('should fail on unexisting user', async () => {
    ...
    try {
      await retrievePendingPractices(fakeId)
  ...
  it('should fail on admin user', async () => {
    try {
      await retrievePendingPractices(adminId)
