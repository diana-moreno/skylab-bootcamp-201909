...
module.exports = function(name = 'index') {
  ...
  return manager ? manager : manager = {
    ...
    // _dirname : la ruta donde estoy. Con esto, está haciendo ruta absoluta, partiendo desde el punto actual hasta donde quiere llegar. Concatena una ruta absoluta on una relativa.
    persist() {
      return fs.writeFile(path.join(__dirname, `../../data/users/${name}.json`), JSON.stringify(this.users, undefined, 4)) // 4 es la indentación (prefiero 2)
